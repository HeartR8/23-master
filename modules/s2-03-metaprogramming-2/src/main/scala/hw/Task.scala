...
import scala.compiletime.{constValue, error, summonFrom, erasedValue}
...
object Loggable {
  extension[A: Loggable] (a: A)
    inline def log(message: String): Unit = {
      println(Json.obj(
        "message" -> Json.fromString(message),
        "position" -> Json.fromString(s"${SourceFilePosition.get.file}:${SourceFilePosition.get.line + 1}"),
        "time" -> Json.fromString(java.time.LocalDateTime.now().toString),
        "context" -> summon[Loggable[A]].jsonLog(a)
      ))
    }
}
