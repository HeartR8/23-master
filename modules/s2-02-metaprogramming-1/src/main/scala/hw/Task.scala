 ...
 */
trait Instances {
  given[A] (using Loggable[A]): Loggable[Option[A]] with {
    def jsonLog(a: Option[A]): Json = a match {
      case Some(x) => summon[Loggable[A]].jsonLog(x)
      case None => Json.Null
    }
  }
...
trait Derivation:
  ...
  inline def summonLabels[T <: Tuple]: List[String] = inline erasedValue[T] match {
    case _: (head *: tail) =>
      constValue[head].asInstanceOf[String] :: summonLabels[tail]
   ...
   */
  inline def summonInst[T <: Tuple]: List[Loggable[?]] = inline erasedValue[T] match {
    case _: (head *: tail) =>
      summonInline[Loggable[head]].asInstanceOf[Loggable[?]] :: summonInst[tail]
    case _: EmptyTuple => Nil
  }
